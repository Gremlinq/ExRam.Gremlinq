...
namespace ExRam.Gremlinq.Providers.Tests
{
    ...
    public class JsonSupportTest
    {
        ...
        public async Task GraphSon3ReferenceVertex()
        {
            ...
            array[0]["name"].ToObject<string[]>(new GraphsonDeserializer(GremlinQueryEnvironment.Default)).Should().BeEquivalentTo("marko");
            array[0]["location"].ToObject<string[]>(new GraphsonDeserializer(GremlinQueryEnvironment.Default)).Should().BeEquivalentTo("san diego", "santa cruz", "brussels", "santa fe");
        }
