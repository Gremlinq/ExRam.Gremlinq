...
namespace ExRam.Gremlinq.Providers.CosmosDb.AspNet.Tests
{
    public class ConfigurationTests
    {
        ...
        [Fact]
        public void Configuration_without_PartitionKey() => new ServiceCollection()
            .AddSingleton<IConfiguration>(new ConfigurationBuilder()
                .AddInMemoryCollection(new Dictionary<string, string?>
                {
                    { "Gremlinq:CosmosDb:Uri", "ws://localhost:8182/" },
        ...
        [Fact]
        public void Configuration_with_PartitionKey_in_configuration() => new ServiceCollection()
            .AddSingleton<IConfiguration>(new ConfigurationBuilder()
                .AddInMemoryCollection(new Dictionary<string, string?>
                {
                    { "Gremlinq:CosmosDb:Uri", "ws://localhost:8182/" },
