...
namespace ExRam.Gremlinq.Providers
{
    public sealed class GraphsonDeserializer : JsonSerializer
    {
        ...
        private sealed class ElementConverter : JsonConverter
        {
            ...
            public override bool CanConvert(Type objectType)
            {
                return !objectType.IsSealed && (typeof(IElement).IsAssignableFrom(objectType) || _model.VertexModel.TryGetFilterLabels(objectType).All(x => x.Length > 0) || _model.EdgeModel.TryGetFilterLabels(objectType).All(x => x.Length > 0));
            }
