...
namespace ExRam.Gremlinq.Core.Serialization
{
    public static class Serializer
    {
        ...
        private static ITransformer AddDefaultStepConverters(this ITransformer serializer) => serializer
            ...
            .Add<ConcatTraversalsStep>((step, env, _, recurse) => CreateInstruction("concat", recurse, env, step.Traversals))
            ...
            .Add<LengthStep>((step, env, _, recurse) => step.Scope.Equals(Scope.Local)
                ? CreateInstruction("length", recurse, env, step.Scope)
                : length)
            ...
            .Add<ReverseStep>((_, _, _, _) => reverse)
            ...
            .Add<SubstringStep>((step, env, _, recurse) =>
            {
                ...
                if (step.Range.End.IsFromEnd)
                {
                    if (endIndex == 0)
                    {
                        if (!step.Range.Start.IsFromEnd || startIndex >= 0)
                            return CreateInstruction("substring", recurse, env, step.Scope, (step.Range.Start.IsFromEnd ? -1 : 1) * startIndex);
                ...
                return CreateInstruction("substring", recurse, env, step.Scope, startIndex, endIndex);
            })
            ...
            .Add<ToLowerStep>((step, env, _, recurse) => step.Scope.Equals(Scope.Local)
                ? CreateInstruction("toLower", recurse, env, step.Scope)
                : toLower)
            .Add<ToUpperStep>((step, env, _, recurse) => step.Scope.Equals(Scope.Local)
                ? CreateInstruction("toUpper", recurse, env, step.Scope)
                : toUpper)
            ...
            .Add<TrimStep>((step, env, _, recurse) => step.Scope.Equals(Scope.Local)
                ? CreateInstruction(nameof(trim), recurse, env, step.Scope)
                : trim)
            .Add<TrimEndStep>((step, env, _, recurse) => step.Scope.Equals(Scope.Local)
                ? CreateInstruction(nameof(rTrim), recurse, env, step.Scope)
                : rTrim)
            .Add<TrimStartStep>((step, env, _, recurse) => step.Scope.Equals(Scope.Local)
                ? CreateInstruction(nameof(lTrim), recurse, env, step.Scope)
                : lTrim)
