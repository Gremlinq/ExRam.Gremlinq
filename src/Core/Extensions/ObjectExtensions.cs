...
namespace ExRam.Gremlinq.Core
{
    ...
    {
        private static readonly ConcurrentDictionary<Type, Func<object, ICachingGremlinQueryEnvironment, SerializationBehaviour, IEnumerable<(Key key, object? value)>>> SerializerDict = new();
            ...
            SerializationBehaviour ignoreMask)
        {
                ...
                .GetOrAdd(
                    ...
                    static closureType =>
                    {
                        ...
                        foreach (var iface in interfaces)
                        {
                            ...
                            {
                                return (Func<object, ICachingGremlinQueryEnvironment, SerializationBehaviour, IEnumerable<(Key key, object? value)>>)typeof(ObjectExtensions)
            ...
            return func(obj, (ICachingGremlinQueryEnvironment)environment, ignoreMask);
        }

        public static object GetId(this object element, ICachingGremlinQueryEnvironment environment)
        {
            var (propertyInfo, _) = environment.GetSerializationData(element.GetType())
        ...
        private static Func<object, ICachingGremlinQueryEnvironment, SerializationBehaviour, IEnumerable<(Key key, object? value)>> CreateSerializeDictionaryFunc<TKey, TValue>() => static (dict, _, _) => SerializeDictionary((IDictionary<TKey, TValue>)dict);
        ...
        private static IEnumerable<(Key key, object? value)> SerializeObject(object obj, ICachingGremlinQueryEnvironment environment, SerializationBehaviour ignoreMask)
        {
            var serializationBehaviourOverrides = environment.Options
                .GetValue(GremlinqOption.TSerializationBehaviourOverrides);

            foreach (var (propertyInfo, metadata) in environment.GetSerializationData(obj.GetType()))
            {
