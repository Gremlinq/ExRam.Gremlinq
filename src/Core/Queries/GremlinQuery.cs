    ...
    internal sealed partial class GremlinQuery<T1, T2, T3, T4> : GremlinQueryBase
    {
        ...
        private GremlinQuery<TNewElement, object, object, IGremlinQueryBase> BothV<TNewElement>() => this
            ...
            .Build(static builder => builder
                ...
                .OfType<object>(TypeArrayCache<TNewElement>.Types, builder.OuterQuery.Environment.Model.VerticesModel)
                .WithNewProjection(Projection.Vertex)
        ...
        private GremlinQuery<TNewElement, object, object, IGremlinQueryBase> E<TNewElement>(ReadOnlySpan<object> ids) => this
            ...
            .Build(
                static (builder, ids) => builder
                    ...
                    .OfType<object>(TypeArrayCache<TNewElement>.Types, builder.OuterQuery.Environment.Model.EdgesModel)
                    .WithNewProjection(Projection.Edge)
                    .BuildAuto<TNewElement>(),
                ids);
        ...
        private TTargetQuery OfType<TNewElement, TTargetQuery>(IGraphElementModel model) where TTargetQuery : IStartGremlinQuery => this
            ...
            .Build(
                static (builder, model) => builder
                    .OfType<T1>(TypeArrayCache<TNewElement>.Types, model)
                    .BuildAs<TTargetQuery>(),
                model);
        ...
        private GremlinQuery<object, object, object, IGremlinQueryBase> Out(Type[]? maybeEdgeTypes) => this
            .Continue()
            .Build(
                static (builder, step) => builder
                    .AddStep(step)
        ...
        private GremlinQuery<TEdge, T1, object, IGremlinQueryBase> OutE<TEdge>(Type[]? maybeEdgeTypes) => this
            .Continue()
            .Build(
                static (builder, step) => builder
                    .AddStep(step)
