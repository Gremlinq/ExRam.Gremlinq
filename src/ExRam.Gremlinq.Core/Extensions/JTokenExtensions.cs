...
namespace ExRam.Gremlinq.Core
{
    internal static class JTokenExtensions
    {
        ...
        public static JObject? TryUnmap(this JObject jObject, IGremlinQueryFragmentDeserializer deserializer, IGremlinQueryEnvironment env)
        {
            if (jObject.TryGetValue("@type", out var nestedType) && "g:Map".Equals(nestedType.Value<string>(), StringComparison.OrdinalIgnoreCase))
            {
                if (jObject.TryGetValue("@value", out var valueToken) && valueToken is JArray mapArray)
                {
                    ...
                    {
                        retObject.Add(mapArray[i * 2].Value<string>(), (JToken)deserializer.TryDeserialize(mapArray[i * 2 + 1], typeof(JToken), env)!);
                    }
        ...
        public static JObject? TryGetElementProperties(this JObject jObject, IGremlinQueryFragmentDeserializer deserializer, IGremlinQueryEnvironment env)
        {
            return jObject.ContainsKey("id") && jObject.TryGetValue("label", out var label) && label.Type == JTokenType.String && jObject["properties"] is JObject propertiesToken
                ? propertiesToken.TryUnmap(deserializer, env) is { } unmapped
                    ? unmapped
                    : propertiesToken
                : null;
        }
